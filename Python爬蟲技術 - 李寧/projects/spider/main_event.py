...
class MyLabel(QLabel):
    def show_analysis_chart(self,value):
        ...
        bar = charts.Bar({'name':'商品月评论数（bar）'})
        bar.add_xaxis(attr).add_yaxis(self.book_title,v)


        bar.render('goods_comment_count_bar.html')

        line = charts.Line({'name':'商品月评论数（line）'})
        line.add_xaxis(attr).add_yaxis(self.book_title, v)
